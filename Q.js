...
function handleCarDeparture() {
    ...
    const carKeys = Object.keys(parkedCars);
    const departingIndex = carKeys.indexOf(plateNumber);

    if (departingIndex === -1) return;

    // Increment arrival and departure counts for all left-side cars
    for (let i = 0; i < departingIndex; i++) {
        const leftCar = carKeys[i];
        if (parkedCars[leftCar]) {
            parkedCars[leftCar].arrivals++;
            parkedCars[leftCar].departures++;
            updateMirror(leftCar);
        }
    }
